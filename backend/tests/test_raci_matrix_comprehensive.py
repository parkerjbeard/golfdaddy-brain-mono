...
class TestRaciService:
    ...
    async def test_bulk_assign_matrix_not_found(self, raci_service):
        ...
        matrix_id = uuid4()
        payload = BulkAssignmentPayload(
            activity_ids=["act1"], role_ids=["role1"], role_type=RaciRoleType.RESPONSIBLE, clear_existing=False
        )
    ...
    async def test_validate_matrix_complete_missing_r_and_a(self, raci_service):
        ...
        matrix = RaciMatrix(
            ...
            roles=[RaciRole(id="role1", name="Role 1", order=1)],
            assignments=[RaciAssignment(activity_id="act1", role_id="role1", role=RaciRoleType.CONSULTED)],
            is_active=True,
        )
    ...
    async def test_validate_matrix_complete_multiple_accountable(self, raci_service):
        ...
        matrix = RaciMatrix(
            ...
            description="Test",
            matrix_type=RaciMatrixType.CUSTOM,
            activities=[RaciActivity(id="act1", name="Activity 1", order=1)],
    ...
    async def test_create_matrix_from_template_success(self, raci_service, sample_user):
        ...
        matrix, warnings = await raci_service.create_matrix_from_template(template_id, name, description, created_by)
    ...
    async def test_create_matrix_from_template_invalid_template(self, raci_service):
        ...
        with pytest.raises(BadRequestError) as exc_info:
            await raci_service.create_matrix_from_template("invalid_template_id", "Test", None, uuid4())
    ...
    async def test_bulk_assign_with_clear_existing(self, raci_service, sample_matrix):
        ...
        matrix_id = sample_matrix.id
        payload = BulkAssignmentPayload(
            activity_ids=["act1"], role_ids=["role1"], role_type=RaciRoleType.INFORMED, clear_existing=True
        )

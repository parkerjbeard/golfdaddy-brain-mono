...
class TestRaciMatrixAPI:
    ...
    def sample_matrix(self):
        ...
        return RaciMatrix(
            ...
            roles=[RaciRole(id="role1", name="Role 1", order=1), RaciRole(id="role2", name="Role 2", order=2)],
            assignments=[
    ...
    def test_update_assignments_success(self, mock_service_class, client, mock_user, auth_headers, sample_matrix):
        ...
        with patch.object(app.dependency_overrides, get_current_user.__name__, return_value=mock_user):
            payload = {
                "assignments": [{"activity_id": "act1", "role_id": "role2", "role": "C", "notes": "Updated assignment"}]
            }

            response = client.patch(
                f"/api/v1/raci-matrices/{sample_matrix.id}/assignments", json=payload, headers=auth_headers
            )
    ...
    def test_update_assignments_with_warnings(self, mock_service_class, client, mock_user, auth_headers, sample_matrix):
        ...
        with patch.object(app.dependency_overrides, get_current_user.__name__, return_value=mock_user):
            payload = {"assignments": [{"activity_id": "invalid_act", "role_id": "role1", "role": "R"}]}
    ...
    def test_update_assignments_matrix_not_found(self, mock_service_class, client, mock_user, auth_headers):
        ...
        with patch.object(app.dependency_overrides, get_current_user.__name__, return_value=mock_user):
            ...
            response = client.patch(f"/api/v1/raci-matrices/{uuid4()}/assignments", json=payload, headers=auth_headers)
    ...
    def test_bulk_assign_success(self, mock_service_class, client, mock_user, auth_headers, sample_matrix):
        ...
        with patch.object(app.dependency_overrides, get_current_user.__name__, return_value=mock_user):
            ...
            response = client.post(
                f"/api/v1/raci-matrices/{sample_matrix.id}/bulk-assign", json=payload, headers=auth_headers
            )
    ...
    def test_validate_complete_valid_matrix(self, mock_service_class, client, mock_user, auth_headers, sample_matrix):
        ...
        with patch.object(app.dependency_overrides, get_current_user.__name__, return_value=mock_user):
            response = client.post(f"/api/v1/raci-matrices/{sample_matrix.id}/validate-complete", headers=auth_headers)
    ...
    def test_get_templates(self, mock_service_class, client, mock_user, auth_headers):
        ...
        mock_templates = [
            RaciMatrixTemplate(
                ...
                activities=[RaciActivity(id="t_act1", name="Template Activity", order=1)],
                roles=[RaciRole(id="t_role1", name="Template Role", order=1)],
                assignments=[RaciAssignment(activity_id="t_act1", role_id="t_role1", role=RaciRoleType.RESPONSIBLE)],
            )
        ]
        mock_service.get_matrix_templates = Mock(return_value=mock_templates)

        with patch.object(app.dependency_overrides, get_current_user.__name__, return_value=mock_user):
            response = client.get("/api/v1/raci-matrices/templates", headers=auth_headers)
    ...
    def test_create_from_template_success(self, mock_service_class, client, mock_user, auth_headers):
        ...
        mock_service = mock_service_class.return_value
        mock_service.create_matrix_from_template = AsyncMock(return_value=(created_matrix, []))

        with patch.object(app.dependency_overrides, get_current_user.__name__, return_value=mock_user):
            response = client.post(
                "/api/v1/raci-matrices/templates/inventory_inbound_template",
                params={"name": "My Process", "description": "Created from template"},
    ...
    def test_create_from_template_invalid_template(self, mock_service_class, client, mock_user, auth_headers):
        ...
        with patch.object(app.dependency_overrides, get_current_user.__name__, return_value=mock_user):
            response = client.post(
                "/api/v1/raci-matrices/templates/invalid_template", params={"name": "Test"}, headers=auth_headers
            )
    ...
    def test_endpoints_require_authentication(self, client):
        ...
        # Test bulk assign
        response = client.post(
            f"/api/v1/raci-matrices/{matrix_id}/bulk-assign",
            json={"activity_ids": [], "role_ids": [], "role_type": "R", "clear_existing": False},
        )
        assert response.status_code == 500
        assert "API key required" in response.text or "authentication" in response.text.lower()

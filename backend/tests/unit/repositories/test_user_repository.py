...
def test_create_user(mock_supabase_client):
    ...
    mock_supabase_client.table.return_value.insert.assert_called_once_with(user_data_to_create.model_dump(exclude_unset=True))
...
def test_get_user_by_id(mock_supabase_client):
    ...
    test_user_id = uuid4()
    mock_user_data = {
        "id": str(test_user_id), "slack_id": "S123", "name": "Found User", "role": "Developer"
    }
...
def test_get_user_by_slack_id(mock_supabase_client):
    ...
    mock_user_data = {
        "id": str(uuid4()), "slack_id": test_slack_id, "name": "Slack User", "role": "Manager"
    }
...
def test_update_user(mock_supabase_client):
    ...
    original_user_data = {
        "id": str(user_id_to_update), "name": "Old Name", "role": UserRole.DEVELOPER.value, "slack_id": "UOld"
    }
    ...
    mock_supabase_client.table.return_value.update.return_value.eq.return_value.execute.return_value = MagicMock(
        data=[updated_user_response_data],
        error=None
    )
...
def test_update_personal_mastery(mock_supabase_client):
    ...
    # Configure mock response for successful update
    mock_supabase_client.table.return_value.update.return_value.eq.return_value.execute.return_value = MagicMock(
        data=[updated_user_response_dict], # Supabase update returns a list
        error=None
    )

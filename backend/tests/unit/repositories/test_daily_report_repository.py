...
from app.models.daily_report import DailyReport, DailyReportCreate, DailyReportUpdate, AiAnalysis
...
async def test_get_daily_report_by_id(report_repository: DailyReportRepository, sample_report_create: DailyReportCreate):
    created_report = await report_repository.create_daily_report(sample_report_create)
...
async def test_get_daily_reports_by_user_and_date(report_repository: DailyReportRepository, sample_user_id: UUID):
    ...
    retrieved_non_existent_date = await report_repository.get_daily_reports_by_user_and_date(user_id, today - timedelta(days=2))
...
async def test_update_daily_report_partial(report_repository: DailyReportRepository, sample_report_create: DailyReportCreate):
    created_report = await report_repository.create_daily_report(sample_report_create)

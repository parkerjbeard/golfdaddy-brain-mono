...
class TestWebhookEndpoints:
    ...
    @patch("app.api.webhooks.settings")
    async def test_github_webhook_no_secret_configured(self, mock_settings, client, github_push_payload):
    ...
    async def test_github_webhook_invalid_signature(
        self, mock_get_db, mock_settings, client, webhook_secret, github_push_payload
    ):
    ...
    async def test_github_webhook_invalid_json(
        self, mock_verify_signature, mock_get_db, mock_settings, client, webhook_secret
    ):
        ...
        response = client.post(
            ...
            headers={"Content-Type": "application/json", "X-Hub-Signature-256": signature, "X-GitHub-Event": "push"},
        )
    ...
    async def test_github_webhook_processing_error(
        ...
        mock_process_event.side_effect = ExternalServiceError(
            service_name="GitHub", original_message="Processing failed"
        )
    ...
    @patch("app.api.webhooks.settings")
    async def test_github_webhook_status_configured(self, mock_settings, client):
    ...
    @patch("app.api.webhooks.settings")
    async def test_github_webhook_status_not_configured(self, mock_settings, client):

...
class TestAuthEndpoints:
    ...
    def mock_supabase(self):
        ...
        mock = Mock()
        mock.auth.get_user.return_value = Mock(
            user=Mock(
                id="test-user-123",
                email="test@example.com"
            )
    ...
    async def test_refresh_token_success(self, mock_user):
        ...
        old_token = jwt.encode(
            {"sub": mock_user.id, "exp": datetime.utcnow() + timedelta(hours=1)},
            secret,
            algorithm="HS256"
        )
        
        with patch('app.config.settings.settings.JWT_SECRET', secret), \
             patch('app.api.auth_endpoints.UserRepository') as mock_repo:
    ...
    def test_create_access_token(self):
        ...
        with patch('app.config.settings.settings.JWT_SECRET', 'test-secret'), \

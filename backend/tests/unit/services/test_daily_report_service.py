...
async def test_submit_daily_report_new(
    ...
    processed_report_mock = DailyReport(
        ...
        ai_analysis=AiAnalysis(summary=f"AI processing pending for: {sample_daily_report_create_data.raw_text_input[:50]}..."),
...
async def test_link_commits_to_report(
    ...
    update_payload = call_args[1]
    assert isinstance(update_payload, DailyReportUpdate)
    assert sorted(update_payload.linked_commit_ids) == all_commits # Ensure unique and sorted for comparison if order doesn't matter

...
from app.core.exceptions import AIIntegrationError, AppExceptionBase, DatabaseError, ResourceNotFoundError
from app.services.doc_generation_service import DocGenerationService
from tests.fixtures.auto_doc_fixtures import QUALITY_TEST_CASES, TEST_CONFIG
...
# Third Heading"""
    ...
    def test_save_doc_reference_database_error(self, service, mock_supabase):
        ...
        mock_supabase.execute.return_value = Mock(error=Mock(message="Update failed"))
    ...
    def test_handle_iteration_success(self, service, mock_supabase, mock_ai_integration):
        ...
        original_doc = {"id": doc_id, "title": "Original Title", "content": "Original content", "feedback_history": []}
...
Detailed error handling explanation..."""
        ...
        mock_ai_integration.update_doc.assert_called_once_with(
            {"original_content": "Original content", "feedback": feedback}
        )
    ...
    def test_generate_documentation_with_custom_title(self, service, mock_supabase, mock_ai_integration):
        ...
        context = {"description": "Database migration guide", "doc_type": "tutorial"}
...
Step 2: Run migration scripts..."""
    ...
    def test_database_error_handling_patterns(self, service, mock_supabase):
        ...
        mock_supabase.execute.return_value = Mock(data=None, error=Mock(message="Connection timeout"))
    ...
    def test_concurrent_documentation_generation(self, service, mock_supabase, mock_ai_integration):
        ...
        contexts = [{"description": f"Service {i}", "doc_type": "api"} for i in range(3)]

        mock_ai_integration.generate_doc.side_effect = [f"# Service {i} Documentation" for i in range(3)]
        mock_supabase.execute.return_value = Mock(data=[{"id": "123"}], error=None)
    ...
    def test_handle_iteration_preserves_metadata(self, service, mock_supabase, mock_ai_integration):
        ...
        original_doc = {
            ...
            "feedback_history": [{"timestamp": "2024-01-02T00:00:00", "feedback": "Initial feedback"}],

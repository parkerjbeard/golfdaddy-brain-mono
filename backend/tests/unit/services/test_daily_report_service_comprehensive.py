...
class TestDailyReportService:
    ...
    async def test_create_daily_report_success(
        self, daily_report_service, mock_daily_report_repo, mock_ai_integration, sample_user
    ):
    ...
    async def test_create_daily_report_duplicate(
        self, daily_report_service, mock_daily_report_repo, sample_user, sample_daily_report
    ):
        """Test duplicate daily report creation prevention"""
        # Arrange
        report_data = DailyReportCreate(
            user_id=sample_user.id, date=datetime.now(timezone.utc).date(), content="Another report"
        )
    ...
    @pytest.mark.asyncio
    async def test_create_daily_report_with_clarification_needed(
        self, daily_report_service, mock_daily_report_repo, mock_ai_integration
    ):
        """Test daily report creation when AI requests clarification"""
        # Arrange
        report_data = DailyReportCreate(
            user_id=uuid4(), date=datetime.now(timezone.utc).date(), content="Did some work"
        )
    ...
    async def test_update_daily_report_success(
        self, daily_report_service, mock_daily_report_repo, mock_ai_integration, sample_daily_report
    ):
    ...
    async def test_get_user_reports_with_date_range(self, daily_report_service, mock_daily_report_repo, sample_user):
        ...
        result = await daily_report_service.get_user_reports(
            user_id=sample_user.id, start_date=start_date, end_date=end_date
        )
    ...
    @pytest.mark.asyncio
    async def test_process_clarification_response(
        self, daily_report_service, mock_daily_report_repo, mock_ai_integration, sample_daily_report
    ):
        ...
        result = await daily_report_service.process_clarification_response(
            report_id=sample_daily_report.id, user_response="I implemented user authentication and fixed security bugs"
        )
    ...
    async def test_get_team_summary(self, daily_report_service, mock_daily_report_repo):
        ...
        # Act
        result = await daily_report_service.get_team_summary(team_id=uuid4(), date=report_date)
    ...
    @pytest.mark.asyncio
    async def test_analyze_report_with_deduplication(
        self, daily_report_service, mock_deduplication_service, mock_commit_repo, sample_user
    ):
        ...
        mock_deduplication_service.find_duplicates.return_value = {
            ...
            "duplicates": [
                {"commit_id": str(commits[0].id), "confidence": 0.95, "reason": "Feature X mentioned in both"}
            ],
        }
    ...
    async def test_ai_analysis_error_handling(self, daily_report_service, mock_daily_report_repo, mock_ai_integration):
        ...
        # Arrange
        report_data = DailyReportCreate(user_id=uuid4(), date=datetime.now(timezone.utc).date(), content="Test content")

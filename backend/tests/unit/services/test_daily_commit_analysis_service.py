...
async def test_analyze_for_report_new_analysis(
    ...
    service.repository.create = AsyncMock(return_value=DailyCommitAnalysis(
        id=uuid4(),
...
@pytest.mark.asyncio
async def test_analyze_for_report_existing_analysis(
    service, sample_user_id, sample_date, sample_daily_report
):
...
@pytest.mark.asyncio
async def test_analyze_for_report_no_commits(
    service, sample_user_id, sample_date, sample_daily_report
):
    ...
    service._create_zero_hour_analysis = AsyncMock(return_value=DailyCommitAnalysis(
        id=uuid4(),
        user_id=sample_user_id,
        analysis_date=sample_date,
...
@pytest.mark.asyncio
async def test_analyze_for_date_automatic(
    service, sample_user_id, sample_date, sample_commits, sample_ai_analysis
):
...
async def test_run_midnight_analysis(service):
    ...
    service.analyze_for_date = AsyncMock(side_effect=[
...
async def test_prepare_analysis_context(service, sample_commits, sample_daily_report, sample_user_id, sample_date):
    ...
    # Execute
    context = await service._prepare_analysis_context(
        sample_commits, sample_daily_report, sample_user_id, sample_date
    )
...
async def test_get_user_analysis_history(service, sample_user_id):
    ...
    mock_analyses = [
        Mock(analysis_date=date(2025, 1, 15)),
        Mock(analysis_date=date(2025, 1, 10))
    ]
    ...
    service.repository.get_user_analyses_in_range.assert_called_once_with(

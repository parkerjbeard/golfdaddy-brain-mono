...
class TestEODReminderService:
    ...
    @pytest.mark.asyncio
    async def test_send_eod_reminders_success(
        self,
        eod_reminder_service,
        mock_user_repo,
        mock_daily_report_repo,
        mock_commit_repo,
        mock_slack_service,
        mock_slack_templates,
        sample_users,
        sample_commits,
    ):
    ...
    @pytest.mark.asyncio
    async def test_send_eod_reminders_dry_run(
        self, eod_reminder_service, mock_user_repo, mock_daily_report_repo, sample_users
    ):
    ...
    @pytest.mark.asyncio
    async def test_send_eod_reminders_with_errors(
        self, eod_reminder_service, mock_user_repo, mock_slack_service, sample_users
    ):
    ...
    @pytest.mark.asyncio
    async def test_schedule_user_reminder_success(
        self, eod_reminder_service, mock_commit_repo, mock_slack_service, mock_slack_templates, sample_users
    ):
        ...
        # Mock template
        mock_slack_templates.eod_reminder.return_value = {"text": "EOD reminder", "blocks": []}
        ...
        with patch("app.services.eod_reminder_service.datetime") as mock_datetime:
            ...
            result = await eod_reminder_service.schedule_user_reminder(
                user=user, reminder_time=reminder_time, timezone_str=timezone_str
            )
    ...
    @pytest.mark.asyncio
    async def test_schedule_user_reminder_next_day(
        self, eod_reminder_service, mock_commit_repo, mock_slack_service, mock_slack_templates, sample_users
    ):
    ...
    async def test_send_daily_eod_reminders_task(self):
        ...
        with patch("app.services.eod_reminder_service.EODReminderService") as mock_service_class:
            ...
            mock_service.send_eod_reminders = AsyncMock(
                return_value={"reminders_sent": 10, "errors": ["error1", "error2"]}
            )
    ...
    @pytest.mark.asyncio
    async def test_reminder_with_commit_context(
        self, eod_reminder_service, mock_commit_repo, mock_slack_templates, sample_commits
    ):
        """Test reminder includes commit context when available"""
        # Arrange
        user = User(id=uuid4(), email="test@example.com", name="Test User", slack_id="U123456", is_active=True)

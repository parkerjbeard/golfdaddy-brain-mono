...
from app.services.deduplication_service import DeduplicationResult, DeduplicationRule, DeduplicationService, WorkType
...
class TestDeduplicationService:
    ...
    @pytest.mark.asyncio
    async def test_find_duplicates_high_confidence(
        self, deduplication_service, mock_ai_integration, sample_commits, sample_daily_report
    ):
        ...
        # Act
        result = await deduplication_service.find_duplicates(commits=sample_commits, daily_report=sample_daily_report)
    ...
    async def test_find_duplicates_no_matches(self, deduplication_service, mock_ai_integration, sample_commits):
        ...
        # Act
        result = await deduplication_service.find_duplicates(commits=sample_commits, daily_report=report)
    ...
    async def test_apply_deduplication_rules(self, deduplication_service):
        ...
        confidence = deduplication_service._apply_rules(
            commit_message="fix: resolve bug", report_content="Updated documentation", base_confidence=0.3
        )
    ...
    async def test_weekly_aggregation(
        self, deduplication_service, mock_commit_repo, mock_daily_report_repo, sample_user
    ):
        ...
        with patch.object(deduplication_service, "find_duplicates", new_callable=AsyncMock) as mock_find_duplicates:
            mock_find_duplicates.return_value = DeduplicationResult(
                duplicates=[{"commit_id": str(commits[0].id), "confidence": 0.9, "hours_duplicated": 2.0}],
    ...
    async def test_deduplication_result_persistence(self, deduplication_service, mock_daily_report_repo):
        ...
        result = DeduplicationResult(
            duplicates=[{"commit_id": str(uuid4()), "confidence": 0.85, "hours_duplicated": 3.0}],
        ...
        # Assert
        mock_daily_report_repo.save_deduplication_result.assert_called_once_with(report_id=report_id, result=result)
    ...
    @pytest.mark.asyncio
    async def test_ai_similarity_error_handling(
        self, deduplication_service, mock_ai_integration, sample_commits, sample_daily_report
    ):
        """Test handling of AI service errors"""
        # Arrange
        mock_ai_integration.calculate_semantic_similarity = AsyncMock(side_effect=Exception("AI service unavailable"))

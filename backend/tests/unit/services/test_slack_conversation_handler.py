...
from app.services.slack_conversation_handler import SlackConversationHandler, ConversationState
                ...
                "values": {
                    "report_content": {
                        "content": {
                            ...
                            "value": "Worked on API integration and documentation"
                        }
                    },
                    "hours_worked": {
                        "hours": {
                    ...
                    "blockers": {
                        "blockers": {
                            "type": "plain_text_input",
                            "value": "Waiting for design review"
                        }
                    }
                }
            },
...
class TestSlackConversationHandler:
    ...
    @pytest.mark.asyncio
    async def test_handle_slash_command_with_text(self, conversation_handler, mock_user_repo, mock_daily_report_service, sample_user, slack_slash_command):
    ...
    @pytest.mark.asyncio
    async def test_handle_view_submission_success(self, conversation_handler, mock_user_repo, mock_daily_report_service, sample_user, slack_view_submission):
    ...
    @pytest.mark.asyncio
    async def test_handle_view_submission_with_clarification(self, conversation_handler, mock_user_repo, mock_daily_report_service, mock_slack_service, sample_user, slack_view_submission):
    ...
    @pytest.mark.asyncio
    async def test_handle_message_event_clarification_response(self, conversation_handler, mock_user_repo, mock_daily_report_service, sample_user, sample_daily_report, slack_event_message):
        ...
        mock_daily_report_service.process_clarification_response.assert_called_once_with(
            report_id=sample_daily_report.id,
            user_response=slack_event_message["text"]
        )
    ...
    @pytest.mark.asyncio
    async def test_start_clarification_conversation(self, conversation_handler, mock_slack_service, mock_slack_templates, sample_user, sample_daily_report):
    ...
    @pytest.mark.asyncio
    async def test_handle_multiple_clarification_rounds(self, conversation_handler, mock_daily_report_service, sample_user, sample_daily_report, slack_event_message):
        ...
        assert conversation_handler.active_conversations[slack_event_message["thread_ts"]].state == "awaiting_clarification"
    ...
    @pytest.mark.asyncio
    async def test_error_handling_in_view_submission(self, conversation_handler, mock_user_repo, mock_daily_report_service, slack_view_submission):

...
class TestSemanticSearchService:
    ...
    async def test_search_documents_with_context(self, service, mock_db, sample_doc_embeddings, sample_code_contexts):
        ...
        service.embedding_service.find_similar_documents = AsyncMock(
            return_value=sample_doc_embeddings
        )
    ...
    async def test_search_documents_without_context(self, service, mock_db, sample_doc_embeddings):
        ...
        assert len(results["results"]) == 2
        assert all("related_code" not in r for r in results["results"])  # Related code not included when include_context=False
    ...
    async def test_analyze_documentation_gaps(self, service, mock_db):
        ...
        gaps = await service.analyze_documentation_gaps(
            db=mock_db,
            repository="test-repo",
            threshold=0.7
        )
    ...
    async def test_suggest_documentation_improvements(self, service, mock_db):
        ...
        suggestions = await service.suggest_documentation_improvements(
            db=mock_db,
            document_id=doc_id
        )

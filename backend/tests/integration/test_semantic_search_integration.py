...
'''
        ...
        services['doc_agent']._gather_context = AsyncMock(return_value={
            'repository': 'test-repo',
...
'''))])
    ...
    async def test_documentation_gap_detection_workflow(self, setup_services, mock_db):
        ...
        # Analyze gaps
        gaps = await services['semantic_search'].analyze_documentation_gaps(
            db=mock_db,
            repository="test-repo"
        )
...
'''
        ...
        prompt = services['doc_agent']._build_context_aware_prompt(diff, {
            'repository': 'test-repo',
            'affected_files': ['/src/payment.py'],
            'related_docs': [],
            'potential_duplicates': [{
        ...
        with patch('app.api.semantic_search.get_db', return_value=mock_db):
            with patch('app.api.semantic_search.EmbeddingService', return_value=services['embedding_service']):
                with patch('app.api.semantic_search.ContextAnalyzer', return_value=services['context_analyzer']):
                    with patch('app.api.semantic_search.SemanticSearchService', return_value=services['semantic_search']):
                        
                        # Mock search results
                        services['semantic_search'].search_documents = AsyncMock(return_value={
                            'results': [{
                                'id': str(uuid.uuid4()),
                                'title': 'Test Document',

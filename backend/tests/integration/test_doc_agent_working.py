...
class TestDocAgentWorking:
    ...
    def test_diff_parsing(self):
        ...
        client = AutoDocClient(
            openai_api_key="test-key", github_token="test-token", docs_repo="test-repo", enable_semantic_search=False
        )
...
+    print("goodbye")"""
    ...
    async def test_database_integration(self):
        ...
        async with SessionLocal() as session:
            ...
            # Query back
            result = await session.execute(text("SELECT * FROM test_approvals WHERE id = :id"), {"id": test_id})
            row = result.fetchone()
    ...
    def test_configuration(self):
        ...
        client = AutoDocClient(openai_api_key="key", github_token="token", docs_repo="repo")

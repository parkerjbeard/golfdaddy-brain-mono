...
class TestAuthIntegration:
    ...
    def auth_headers(self):
        ...
        secret = "test-secret"
        token = jwt.encode(
            {"sub": "test-user-123", "exp": datetime.utcnow() + timedelta(hours=1)}, secret, algorithm="HS256"
        )
    ...
    def test_login_with_token(self, mock_get_user, mock_supabase, client, mock_user):
        ...
        mock_supabase_client.auth.get_user.return_value = Mock(user=Mock(id="test-user-123", email="test@example.com"))
        mock_supabase.return_value = mock_supabase_client

        # Mock user repository
        mock_get_user.return_value = mock_user

        response = client.post("/auth/token", json={"token": "valid-supabase-token"})
    ...
    def test_login_with_invalid_token(self, client):
        ...
        with patch("app.config.supabase_client.get_supabase_client") as mock_supabase:
            ...
            response = client.post("/auth/token", json={"token": "invalid-token"})
    ...
    def test_validate_token(self, mock_get_user, mock_api_key, client, mock_user):
        ...
        response = client.get("/auth/validate", headers=self.auth_headers())
    ...
    def test_auth_flow_integration(self, client):
        ...
        with (
            patch("app.config.supabase_client.get_supabase_client") as mock_supabase,
            ...
            mock_supabase_client = Mock()
            mock_supabase_client.auth.get_user.return_value = Mock(user=Mock(id=mock_user.id, email=mock_user.email))
            mock_supabase.return_value = mock_supabase_client
            ...
            login_response = client.post("/auth/token", json={"token": "supabase-token"})
            ...
            with (
                patch("app.middleware.api_key_auth.ApiKeyMiddleware.is_valid_api_key", return_value=True),
                patch("app.config.settings.settings.JWT_SECRET", "test-secret"),
            ):

                user_response = client.get(
                    "/api/v1/users/me", headers={"Authorization": f"Bearer {access_token}", "X-API-Key": "test-api-key"}
                )

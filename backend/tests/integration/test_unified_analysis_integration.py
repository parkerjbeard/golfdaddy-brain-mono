...
class TestUnifiedAnalysisIntegration:
    ...
    @pytest.mark.asyncio
    async def test_full_eod_submission_flow(self, mock_supabase_client, sample_user, mock_ai_response):
        ...
        with patch("app.core.supabase_client.get_supabase_client", return_value=mock_supabase_client):
            ...
            with patch("app.integrations.ai_integration.AsyncOpenAI") as mock_openai:
                ...
                result = await service.analyze_daily_work(sample_user.id, analysis_date)
    ...
    @pytest.mark.asyncio
    async def test_slack_eod_triggers_unified_analysis(self, mock_supabase_client, sample_user, mock_ai_response):
        ...
        with patch("app.core.supabase_client.get_supabase_client", return_value=mock_supabase_client):
            ...
            with patch("app.services.slack_conversation_handler.UnifiedDailyAnalysisService") as mock_unified_service:
                ...
                view_data = {
                    "state": {
                        "values": {
                            "report_block": {"report_input": {"value": "Worked on OAuth authentication and fixed bugs"}}
                        }
                    }
                }

                # Execute
                result = await slack_handler.handle_modal_submission(sample_user.slack_id, view_data)

                # Verify unified analysis was triggered
                mock_service_instance.analyze_daily_work.assert_called_once_with(
                    user_id=sample_user.id, date=date.today(), eod_report=created_report
                )
    ...
    @pytest.mark.asyncio
    async def test_commit_webhook_triggers_analysis(self, mock_supabase_client, sample_user, mock_ai_response):
        ...
        with patch("app.core.supabase_client.get_supabase_client", return_value=mock_supabase_client):
            ...
            with patch("app.integrations.ai_integration.AsyncOpenAI") as mock_openai:
                ...
                today_commits = await commit_repo.get_commits_by_user_date_range(
                    sample_user.id, datetime.now(timezone.utc).replace(hour=0, minute=0), datetime.now(timezone.utc)
                )
    ...
    @pytest.mark.asyncio
    async def test_weekly_aggregate_with_unified_analysis(self, mock_supabase_client, sample_user):
    ...
    @pytest.mark.asyncio
    async def test_error_recovery_and_retry(self, mock_supabase_client, sample_user, mock_ai_response):

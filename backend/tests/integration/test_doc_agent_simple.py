...
class TestDocAgentSimple:
    ...
    async def test_complete_workflow_with_mocks(self, db_session):
        ...
        with (
            patch("app.doc_agent.client.AsyncOpenAI") as mock_openai_class,
            ...
            approval_id = await client.propose_via_slack(
                diff=diff, patch="patch content", commit_hash="commit123", commit_message="Test commit", db=db_session
            )
    ...
    async def test_error_handling(self, db_session):
        ...
        with (
            ...
            approval_id = await client.propose_via_slack(diff="diff", patch="patch", commit_hash="hash", db=db_session)
    ...
    async def test_database_operations(self, db_session):
        ...
        result = await db_session.execute(
            text("SELECT * FROM doc_approvals WHERE commit_hash = :hash"), {"hash": "test123"}
        )
        row = result.fetchone()
    ...
    async def test_concurrent_operations(self, db_session):
        ...
        with patch("app.doc_agent.client.AsyncOpenAI") as mock_openai_class:
            ...
            tasks = [client.analyze_diff(f"diff {i}") for i in range(5)]

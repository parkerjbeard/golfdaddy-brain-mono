...
from app.models.raci_matrix import RaciMatrixTemplate, RaciMatrixType, RaciRoleType
    ...
    def test_all_templates_have_valid_assignments(self):
        ...
        for template in templates:
            ...
            for assignment in template.assignments:
                assert (
                    assignment.activity_id in activity_ids
                ), f"Invalid activity ID {assignment.activity_id} in template {template.template_id}"
                assert (
                    assignment.role_id in role_ids
                ), f"Invalid role ID {assignment.role_id} in template {template.template_id}"
                assert assignment.role in [
                    RaciRoleType.RESPONSIBLE,
                    RaciRoleType.ACCOUNTABLE,
                    RaciRoleType.CONSULTED,
                    RaciRoleType.INFORMED,
                ]
    ...
    def test_all_activities_have_r_and_a_assignments(self):
        ...
        for template in templates:
            for activity in template.activities:
                ...
                # Check for R and A
                assert (
                    RaciRoleType.RESPONSIBLE in assignment_roles
                ), f"Activity {activity.name} in template {template.template_id} has no Responsible assignment"
                assert (
                    RaciRoleType.ACCOUNTABLE in assignment_roles

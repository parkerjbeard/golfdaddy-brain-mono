#!/usr/bin/env python3
"""
Simplified import script that directly saves commits without user creation.
"""
...
async def import_github_analysis(json_file_path: str):
    ...
    for day_summary in daily_summaries:
        ...
        # Import each commit analysis WITHOUT requiring user
        for analysis in analyses:
            ...
            try:
                ...
                print(f"  ⚠️  Could not check existing commits: {e}")
            ...
            # Create commit record (without author_id)
            commit_data = Commit(
                ...
                author_id=None,  # No user reference
            ...
            try:
                ...
                print(f"  ✅ Imported {commit_hash[:8]}: {hours}h, impact {impact}")
            except Exception as e:
...
async def main():
    ...
    if imported > 0:
        ...
        print(f"\nNote: Commits were imported without user associations.")
        print(f"User mapping can be added later if needed.")

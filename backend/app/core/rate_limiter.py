...
class TokenBucketRateLimiter:
    ...
    def __init__(self, config: RateLimitConfig):
        ...
        logger.info(
            f"Initialized rate limiter '{config.name}' with "
    ...
    async def acquire(self, tokens: int = 1) -> None:
        ...
        async with self._lock:
            ...
            if self.tokens >= tokens:
                self.tokens -= tokens
                logger.debug(f"Acquired {tokens} tokens for '{self.config.name}', " f"remaining: {self.tokens}")
            else:
...
class SlidingWindowRateLimiter:
    ...
    def __init__(self, config: RateLimitConfig):
        ...
        logger.info(
            f"Initialized sliding window rate limiter '{config.name}' with " f"{config.requests_per_hour} requests/hour"
        )
    ...
    async def acquire(self, requests: int = 1) -> None:
        ...
        async with self._lock:
            ...
            logger.debug(
                f"Acquired {requests} requests for '{self.config.name}', " f"total in window: {len(self.request_times)}"
            )

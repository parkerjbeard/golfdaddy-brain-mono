...
class CircuitBreakerOpenError(Exception):
    ...
    def __init__(self, circuit_name: str, failure_count: int, timeout: int):
        ...
        super().__init__(
            f"Circuit breaker '{circuit_name}' is OPEN. " f"Failed {failure_count} times. Retry after {timeout}s."
        )
...
class CircuitBreaker:
    ...
    def __init__(self, config: CircuitBreakerConfig):
        ...
        logger.info(
            f"Initialized circuit breaker '{config.name}' with "
        ...
        async with self._lock:
            if self.state == CircuitState.OPEN:
                if self._should_attempt_reset():
                    ...
                    raise CircuitBreakerOpenError(self.config.name, self.failure_count, self.config.timeout)
    ...
    async def _on_failure(self):
        ...
        async with self._lock:
            ...
            if self.state == CircuitState.HALF_OPEN:
                ...
                self.success_count = 0
                logger.warning(
                    f"Circuit breaker '{self.config.name}' failed recovery test, " f"returning to OPEN state"
                )
            elif self.state == CircuitState.CLOSED and self.failure_count >= self.config.failure_threshold:
                # Too many failures - open circuit
                self.state = CircuitState.OPEN
                logger.error(f"Circuit breaker '{self.config.name}' OPENED after " f"{self.failure_count} failures")

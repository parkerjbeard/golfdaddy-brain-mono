...
from app.core.exceptions import ExternalServiceError, ResourceNotFoundError, ValidationError
from app.services.doc_quality_service import QualityLevel, QualityMetrics
...
class DocApprovalService:
    ...
    async def create_approval_request(
        ...
        logger.info(
            f"Created approval request {request_id} for document {document_id} " f"(auto_approved: {auto_approve})"
        )
    ...
    async def submit_review(
        ...
        decision: str,
        comments: str,
        reviewer_role: ReviewerRole = ReviewerRole.TECHNICAL_REVIEWER,
    ) -> ApprovalRequest:
        """
    ...
    async def get_pending_approvals_for_user(self, user_id: str) -> List[ApprovalRequest]:
        ...
        try:
            ...
            response = (
                self.supabase.table("doc_approval_requests")
                .select("*")
                .in_("status", [ApprovalStatus.PENDING.value, ApprovalStatus.NEEDS_REVIEW.value])
                .execute()
            )
        ...
        try:
            ...
            for row in response.data:
                ...
                self.supabase.table("doc_approval_requests").update(
                    {"status": ApprovalStatus.EXPIRED.value, "updated_at": now.isoformat()}
        ...
        try:
            ...
            message = {
                ...
                "blocks": [
                    {
                        "type": "header",
                        "text": {"type": "plain_text", "text": f"📝 Documentation Review: {request.title}"},
                    },
                    {
                        "type": "section",
                        "fields": [
                            {"type": "mrkdwn", "text": f"*Type:* {request.doc_type}"},
                            {"type": "mrkdwn", "text": f"*Author:* <@{request.author_id}>"},
                            {
                                "type": "mrkdwn",
                                "text": f"*Expires:* {request.expires_at.strftime('%Y-%m-%d %H:%M') if request.expires_at else 'No expiration'}",
                            },
                            {"type": "mrkdwn", "text": quality_info if quality_info else "*Quality:* Not assessed"},
                        ],
                    },
                    {
                        "type": "section",
    ...
    async def _send_review_notifications(self, request: ApprovalRequest, review: ReviewDecision) -> None:
        ...
        try:
            decision_emoji = {"approve": "✅", "reject": "❌", "request_changes": "🔄"}
            ...
            if review.comments:
                message["blocks"].append(
                    {"type": "section", "text": {"type": "mrkdwn", "text": f"*Comments:*\n{review.comments}"}}
                )

...
from app.core.exceptions import BadRequestError  # For role/validation type issues
...
class PersonalMasteryService:
    ...
    async def get_mastery_tasks(self, manager_id: UUID) -> Optional[Dict[str, Any]]:
        ...
        if manager.role != UserRole.MANAGER:
            logger.warning(
                f"User {manager_id} is not a manager. Cannot retrieve mastery tasks specifically for managers."
            )
    ...
    async def send_mastery_reminder(self, manager_id: UUID):
        ...
        try:
            ...
            logger.error(
                f"Failed to send mastery reminder notification to manager {manager_id}: {e_notify}", exc_info=True
            )

...
from app.core.exceptions import AppExceptionBase  # General fallback
...
class DocGenerationService:
    ...
    def generate_documentation(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            ai_input = {"description": description, "files": file_references, "doc_type": doc_type}
            ...
            return {"doc_id": doc_id, "title": title, "content": generated_content, "generated_at": timestamp}
    ...
    def handle_iteration(self, doc_id: str, feedback: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if (
                not update_response.data
            ):  # For some clients, data might be empty on successful update if returning='minimal'
                logger.warning(
                    f"Supabase update for doc {doc_id} returned no data. Assuming success if no error object."
                )
    ...
    def _append_feedback_history(
        self, existing_history: List[Dict[str, Any]], new_feedback: str
    ) -> List[Dict[str, Any]]:
        """Append new feedback to the document's feedback history."""
        history_item = {"timestamp": datetime.now().isoformat(), "feedback": new_feedback}

...
from app.services.doc_cache_service import cache_service, cached
...
class DocQualityService:
    ...
    async def validate_documentation(
        self, content: str, doc_type: str = "general", context: Optional[Dict[str, Any]] = None
    ) -> QualityMetrics:
        """
        ...
        try:
            ...
            # Combine results
            metrics = self._combine_quality_metrics(content, rule_results, ai_metrics, readability_score, word_count)
    ...
    def _apply_validation_rules(self, content: str) -> Dict[str, Any]:
        ...
        results = {"issues": [], "suggestions": [], "scores": {}}
    ...
    async def _ai_quality_assessment(
        self, content: str, doc_type: str, context: Optional[Dict[str, Any]]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if str(self.openai_model).startswith("gpt-5"):
                response = await self.circuit_breaker.call(
                    ...
                    input=[
                        {
                            "role": "system",
                            "content": "You are an expert technical writing analyst. Evaluate documentation quality and provide detailed feedback in JSON format.",
                        },
                ...
                ai_response = json.loads((content_text or "{}").strip())
            else:
                response = await self.circuit_breaker.call(
            ...
            return {
                "scores": {"clarity": 75, "accuracy": 80, "grammar": 85},
                "issues": ["AI assessment unavailable"],
                "suggestions": ["Manual review recommended"],
            }

    def _create_quality_assessment_prompt(self, content: str, doc_type: str, context: Optional[Dict[str, Any]]) -> str:
    ...
    def _combine_quality_metrics(
        ...
        completeness_score = self._weighted_average(
            [(rule_scores.get("completeness", 80), 0.6), (ai_scores.get("completeness", 80), 0.4)]
        )

        clarity_score = self._weighted_average([(ai_scores.get("clarity", 75), 0.7), (readability_score, 0.3)])

        accuracy_score = ai_scores.get("accuracy", 80)

        consistency_score = self._weighted_average(
            [(rule_scores.get("consistency", 80), 0.5), (ai_scores.get("consistency", 80), 0.5)]
        )
    ...
    def should_approve_automatically(self, metrics: QualityMetrics, doc_type: str) -> bool:
        ...
        return (
            metrics.overall_score >= threshold
            and metrics.level in [QualityLevel.EXCELLENT, QualityLevel.GOOD]
            and all(
                score >= 70
                for score in [
            ...
            if str(self.openai_model).startswith("gpt-5"):
                response = await self.circuit_breaker.call(
                    ...
                    input=[
                        {
                            "role": "system",
                            "content": "You are a technical writing expert. Provide specific, actionable suggestions for improving documentation.",
                        },
                        {"role": "user", "content": prompt},
                    ],
                )
                suggestions_text = (getattr(response, "output_text", None) or "").strip()
                if not suggestions_text and hasattr(response, "choices") and response.choices:

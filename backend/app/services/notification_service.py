...
class NotificationService:
    ...
    async def _get_slack_user_id(self, user: User, db: Any = None) -> Optional[str]:
        ...
        if hasattr(user, "github_username") and user.github_username:
            slack_user = await self.slack_service.find_user_by_github_username(user.github_username)
            if slack_user:
    ...
    async def notify_task_created(self, task: Any, creator: Optional[User] = None, db: Any = None):
        logger.debug(f"Task created notification skipped for task {getattr(task, 'id', 'unknown')}")
        return

    async def notify_milestone_tracking(self, manager: User, milestone: Dict[str, Any]):
        logger.debug(f"Milestone tracking notification skipped for manager {manager.id if manager else 'unknown'}")
        return

    async def notify_task_achievement(self, manager: User, achievement: Dict[str, Any]):
        logger.debug(f"Task achievement notification skipped for manager {manager.id if manager else 'unknown'}")
        return
    ...
    async def notify_daily_task_followup(self, manager: User, task: Dict[str, Any]):
        logger.debug(f"Daily task followup notification skipped for manager {manager.id if manager else 'unknown'}")
        return

    async def blocked_task_alert(
        self, task_id: UUID, reason: str, blocking_user: Optional[User] = None, db: Any = None
    ):
        logger.debug(f"Task blocked alert skipped for task {task_id}")
        return
    ...
    async def send_development_plan_notification(
        ...
        logger.debug(f"Development plan notification skipped for manager {manager_user_id}")
        return

    async def notify_task_escalation_fallback(self, task: Any, escalated_to_user: User, reason_summary: str):
        logger.debug(f"Task escalation notification skipped for task {task.id if task else 'unknown'}")
        return
    ...
    async def send_documentation_proposal(
        ...
        logger.debug(f"Documentation proposal notification skipped for {github_username}")
        return
    ...
    async def send_commit_analysis_summary(
        ...
        logger.debug(f"Commit analysis notification skipped for {github_username}")
        return

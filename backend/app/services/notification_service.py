...
class NotificationService:
    ...
    async def notify_milestone_tracking(self, manager: User, milestone: Dict[str, Any]):
        ...
        logger.debug(f"Milestone tracking notification skipped for manager {manager.id if manager else 'unknown'} - notifications disabled")
        return
    ...
    async def notify_task_achievement(self, manager: User, achievement: Dict[str, Any]): 
        ...
        logger.debug(f"Task achievement notification skipped for manager {manager.id if manager else 'unknown'} - notifications disabled")
        return
    ...
    async def notify_manager_mastery_feedback(self, manager: User, feedback: Dict[str, Any]):
        ...
        logger.debug(f"Manager mastery feedback notification skipped for manager {manager.id if manager else 'unknown'} - notifications disabled")
        return
    ...
    async def notify_daily_task_followup(self, manager: User, task: Dict[str, Any]):
        ...
        logger.debug(f"Daily task followup notification skipped for manager {manager.id if manager else 'unknown'} - notifications disabled")
        return
    ...
    async def notify_task_escalation_fallback(
        ...
        logger.debug(f"Task escalation notification skipped for task {task.id if task else 'unknown'} - notifications disabled")
        return

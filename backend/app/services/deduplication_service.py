...
class DeduplicationService:
    ...
    async def deduplicate_daily_report(
        self,
        report: DailyReport,
        user_commits: Optional[List[Commit]] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.info(f"Deduplication complete for report {report.id}: "
    ...
    async def _find_matches(
        self,
        commit_items: List[WorkItem],
        report_items: List[WorkItem]
    ...
    async def _check_similarity(
        ...
        try:
            ...
            response = await self.ai_integration.analyze_text(
                prompt,
                model=settings.DEDUPLICATION_MODEL if hasattr(settings, 'DEDUPLICATION_MODEL') else None
            )
            
            # Parse response
            if response and response.get('is_match', False):
    ...
    async def generate_weekly_aggregate(
        self,
        user_id: str,
        week_start: datetime,
        week_end: datetime
            ...
            while current_date <= week_end_date:
                ...
                if report:
                    ...
                    day_commit_hours = sum(
                        c.ai_analysis.get('estimated_hours', 0.5) if c.ai_analysis else 0.5

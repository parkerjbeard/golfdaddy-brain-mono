...
from app.models.raci_matrix import (
    ...
    UpdateAssignmentsPayload, BulkAssignmentPayload, RaciMatrixTemplate
)
...
class RaciService:
    ...
    async def validate_matrix_complete(self, matrix_id: UUID) -> Tuple[bool, Dict[str, Any]]:
        ...
        for activity in matrix.activities:
            ...
            if accountable_count > 1:
                result["warnings"].append(f"Activity '{activity.name}' has {accountable_count} Accountable assignments (should have only 1)")
    ...
    def get_matrix_templates(self) -> List[RaciMatrixTemplate]:
        ...
        templates.append(RaciMatrixTemplate(
            ...
            activities=[
                RaciActivity(id="recv_1", name="Receive Shipment", description="Physical receipt of inventory shipment", order=1),
                RaciActivity(id="recv_2", name="Verify Contents", description="Check shipment against purchase order", order=2),
                RaciActivity(id="recv_3", name="Quality Inspection", description="Inspect items for damage or defects", order=3),
                RaciActivity(id="recv_4", name="Update Inventory System", description="Enter received items into inventory management system", order=4),
                RaciActivity(id="recv_5", name="Store Items", description="Put away items in designated warehouse locations", order=5),
                RaciActivity(id="recv_6", name="Report Discrepancies", description="Document and report any issues found", order=6)
            ],
            roles=[
                RaciRole(id="role_wh_mgr", name="Warehouse Manager", title="Warehouse Operations Manager", is_person=False, order=1),
                RaciRole(id="role_recv_clerk", name="Receiving Clerk", title="Inventory Receiving Specialist", is_person=False, order=2),
                RaciRole(id="role_qa", name="QA Inspector", title="Quality Assurance Inspector", is_person=False, order=3),
                RaciRole(id="role_inv_ctrl", name="Inventory Controller", title="Inventory Control Specialist", is_person=False, order=4),
                RaciRole(id="role_purchasing", name="Purchasing", title="Purchasing Department", is_person=False, order=5)
            ],
            assignments=[
                # Receive Shipment
        ...
        templates.append(RaciMatrixTemplate(
            ...
            activities=[
                RaciActivity(id="sb_1", name="Identify Issue", description="Detect and document ShipBob fulfillment issue", order=1),
                RaciActivity(id="sb_2", name="Assess Impact", description="Evaluate customer and business impact", order=2),
                RaciActivity(id="sb_3", name="Contact ShipBob", description="Initiate communication with ShipBob support", order=3),
                RaciActivity(id="sb_4", name="Implement Resolution", description="Execute agreed resolution steps", order=4),
                RaciActivity(id="sb_5", name="Customer Communication", description="Update affected customers", order=5),
                RaciActivity(id="sb_6", name="Monitor Resolution", description="Track resolution progress and effectiveness", order=6)
            ],
            roles=[
                RaciRole(id="role_ops_mgr", name="Operations Manager", title="Operations Manager", is_person=False, order=1),
                RaciRole(id="role_cs_rep", name="Customer Service Rep", title="Customer Service Representative", is_person=False, order=2),
                RaciRole(id="role_fulfillment", name="Fulfillment Coordinator", title="Fulfillment Coordinator", is_person=False, order=3),
                RaciRole(id="role_cs_mgr", name="CS Manager", title="Customer Service Manager", is_person=False, order=4)
            ],
            assignments=[
                # Identify Issue
        ...
        templates.append(RaciMatrixTemplate(
            ...
            activities=[
                RaciActivity(id="dc_1", name="Define Requirements", description="Specify data collection requirements and objectives", order=1),
                RaciActivity(id="dc_2", name="Design Collection Method", description="Create data collection methodology and tools", order=2),
                RaciActivity(id="dc_3", name="Collect Data", description="Execute data collection according to plan", order=3),
                RaciActivity(id="dc_4", name="Validate Data", description="Verify data quality and completeness", order=4),
                RaciActivity(id="dc_5", name="Analyze Data", description="Perform data analysis and generate insights", order=5),
                RaciActivity(id="dc_6", name="Report Findings", description="Create and distribute data reports", order=6)
            ],
            roles=[
                RaciRole(id="role_data_mgr", name="Data Manager", title="Data Management Lead", is_person=False, order=1),
                RaciRole(id="role_analyst", name="Data Analyst", title="Data Analyst", is_person=False, order=2),
                RaciRole(id="role_bus_owner", name="Business Owner", title="Business Process Owner", is_person=False, order=3),
    ...
    async def create_matrix_from_template(self, template_id: str, name: str, description: Optional[str], created_by: UUID) -> Tuple[RaciMatrix, List[str]]:

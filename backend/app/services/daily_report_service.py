...
from app.core.exceptions import AIIntegrationError  # If AI service fails explicitly
from app.core.exceptions import BadRequestError  # For bad input data if not caught by Pydantic
from app.core.exceptions import DatabaseError  # For repository/DB issues
...
class DailyReportService:
    def __init__(self):
        ...
        self.ai_integration = AIIntegrationV2()  # Use unified AI integration (V2)
    ...
    async def submit_daily_report(self, report_data: DailyReportCreate, current_user_id: UUID) -> DailyReport:
        ...
        report_create_with_user = DailyReportCreate(user_id=current_user_id, raw_text_input=report_data.raw_text_input)

        # Check if a report for this user and date already exists
        now = datetime.now(timezone.utc)
        existing_report = await self.report_repository.get_daily_reports_by_user_and_date(current_user_id, now)
        if existing_report:
            ...
            try:
                ...
                if "duplicate key" in error_msg or "unique constraint" in error_msg or "unique_violation" in error_msg:
                    ...
                    if existing_report:
                        ...
                        logger.error(
                            f"Could not find report after unique constraint violation for user {current_user_id}"
                        )
    ...
    async def get_user_report_for_date(
        self, user_id: UUID, report_date: datetime, user_timezone: Optional[str] = None
    ...
    async def get_all_reports_paginated(self, page: int = 1, page_size: int = 10) -> Dict[str, Any]:
        ...
        # Get paginated reports
        reports = await self.report_repository.get_all_daily_reports_paginated(offset=offset, limit=page_size)
    ...
    async def process_report_with_ai(self, report: DailyReport) -> DailyReport:
        ...
        try:
            ...
            ai_analysis_dict = await self.ai_integration.analyze_eod_report(report.raw_text_input)

            if ai_analysis_dict.get("error"):
                logger.error(
                    f"AI analysis for EOD report {report.id} returned an error: {ai_analysis_dict.get('error')}"
                )
    ...
    async def update_daily_report(
        self, report_id: UUID, report_data: DailyReportUpdate, user_id: UUID
    ) -> Optional[DailyReport]:
        """
    ...
    async def update_report_assessment(
        self, report_id: UUID, assessment_notes: str, final_hours: float
    ) -> Optional[DailyReport]:
        """Allows a manager or system to add overall assessment and final hours."""
        update_data = DailyReportUpdate(overall_assessment_notes=assessment_notes, final_estimated_hours=final_hours)
    ...
    async def get_weekly_hours_summary(self, user_id: UUID, week_start: datetime) -> Dict[str, Any]:
        ...
        commits = await self.commit_repository.get_commits_by_user_id(user_id, start_date=week_start, end_date=week_end)
    ...
    async def handle_slack_conversation(
        self, report_id: UUID, user_message: str, slack_thread_ts: str
    ) -> Dict[str, Any]:
        """
        ...
        if not report:
            return {"error": "Report not found", "message": "I couldn't find the report you're referring to."}
        ...
        try:
            ...
            update_data = DailyReportUpdate(conversation_state=conversation_state, slack_thread_ts=slack_thread_ts)

...
class DocDiffService:
    ...
    async def create_version(self, document_id: str, content: str, title: str,
                           author_id: str, commit_message: Optional[str] = None,
    ...
    async def list_versions(self, document_id: str, limit: int = 20) -> List[DocumentVersion]:
        ...
        try:
            response = self.supabase.table("document_versions").select("*").eq(
                "document_id", document_id
    ...
    async def generate_diff(self, from_version_id: str, to_version_id: str,
                          diff_format: DiffFormat = DiffFormat.UNIFIED) -> DocumentDiff:
        """
        ...
        try:
            ...
            logger.info(f"Generated diff between versions {from_version.version_number} and {to_version.version_number}")
            return diff
    ...
    async def preview_changes(self, current_content: str, proposed_content: str,
                            format_type: DiffFormat = DiffFormat.SIDE_BY_SIDE) -> Dict[str, Any]:
        """
    ...
    async def rollback_to_version(self, document_id: str, target_version_id: str,
                                author_id: str, rollback_reason: str = "") -> DocumentVersion:
        """
    ...
    async def get_rollback_history(self, document_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            response = self.supabase.table("document_rollbacks").select("*").eq(
                "document_id", document_id
    ...
    async def _get_next_version_number(self, document_id: str) -> int:
        ...
        try:
            response = self.supabase.table("document_versions").select("version_number").eq(
                "document_id", document_id
    ...
    def _format_diff_side_by_side(self, chunks: List[DiffChunk], old_content: str, new_content: str) -> List[Dict[str, Any]]:
    ...
    async def _cleanup_old_versions(self, document_id: str) -> None:
        ...
        try:
            # Get version count
            response = self.supabase.table("document_versions").select("id").eq(
                "document_id", document_id
            ...
            if len(response.data) > self.max_versions_per_doc:
                ...
                self.supabase.table("document_versions").delete().eq(
                    "document_id", document_id
    ...
    async def _log_rollback(self, document_id: str, target_version_id: str,
                          new_version_id: str, author_id: str, reason: str) -> None:

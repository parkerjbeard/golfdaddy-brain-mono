...
class DailyCommitAnalysisService:
    ...
    async def analyze_for_report(
        ...
        try:
            ...
            if not commits:
                ...
                return await self._create_zero_hour_analysis(
                    user_id, report_date, daily_report.id, "with_report"
                )
            
            # Prepare context for AI analysis
            context = await self._prepare_analysis_context(
                commits, daily_report, user_id, report_date
            )
            ...
            raise ExternalServiceError(
                service_name="Daily Commit Analysis",
                original_message=f"Failed to analyze commits: {str(e)}"
            )
    
    async def analyze_for_date(
        self,
        user_id: UUID,
        analysis_date: date
    ) -> DailyCommitAnalysis:
        ...
        try:
            ...
            context = await self._prepare_analysis_context(
                commits, None, user_id, analysis_date
            )
            ...
            logger.info(f"✓ Automatic daily analysis created: {analysis.id} with {analysis.total_estimated_hours} hours")
            return analysis
    ...
    async def get_user_analysis_history(
        ...
        try:
            return await self.repository.get_user_analyses_in_range(
    ...
    async def _get_user_commits_for_date(
        self,
        user_id: UUID,
        commit_date: date
    ) -> List[Commit]:
    ...
    async def _link_commits_to_analysis(
        self,
        commits: List[Commit],
        analysis_id: UUID
    ) -> None:

...
from app.models.doc_embeddings import DocEmbedding, CodeContext, DocRelationship
...
class SemanticSearchService:
    ...
    async def search_documentation(
        ...
        similar_docs = await self.embedding_service.find_similar_documents(
            db, query, repository, limit, threshold=0.5
        )
    ...
    async def analyze_documentation_gaps(
        ...
        if code_contexts:
            gaps['coverage_summary']['coverage_percentage'] = (
                gaps['coverage_summary']['documented_files'] / 
                gaps['coverage_summary']['total_files'] * 100
            )
        
        # Generate suggestions for top undocumented files
        undocumented_sorted = sorted(
    ...
    async def _count_total_documents(
        self,
        db: AsyncSession,
        repository: Optional[str] = None
    ) -> int:

...
class ArchiveService:
    ...
    def __init__(self, supabase: Client):
        ...
        self.retention_policies = {
            "daily_reports": RetentionPolicy(
                ...
                retention_months=int(
                    settings.DAILY_REPORTS_RETENTION_MONTHS
                    if hasattr(settings, "DAILY_REPORTS_RETENTION_MONTHS")
                    else 12
                ),
            ...
            "commits": RetentionPolicy(
                table_name="commits",
                retention_months=int(
                    settings.COMMITS_RETENTION_MONTHS if hasattr(settings, "COMMITS_RETENTION_MONTHS") else 24
                ),
                archive_method="soft_delete",
            ...
            "tasks": RetentionPolicy(
                table_name="tasks",
                retention_months=int(
                    settings.COMPLETED_TASKS_RETENTION_MONTHS
                    if hasattr(settings, "COMPLETED_TASKS_RETENTION_MONTHS")
                    else 6
                ),
                archive_method="soft_delete",
            ...
            "docs": RetentionPolicy(
                table_name="docs",
                retention_months=int(
                    settings.DOCS_RETENTION_MONTHS if hasattr(settings, "DOCS_RETENTION_MONTHS") else 18
                ),
                archive_method="soft_delete",
                date_column="updated_at",
            ),
    ...
    async def archive_old_data(self, table_name: Optional[str] = None, dry_run: bool = True) -> Dict[str, Any]:
        ...
        try:
            policies_to_process = (
                [self.retention_policies[table_name]] if table_name else self.retention_policies.values()
            )
    ...
    async def _soft_delete_records(
        self, policy: RetentionPolicy, cutoff_date: datetime, dry_run: bool
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if dry_run:
                ...
                response = (
                    self.supabase.table(policy.table_name)
    ...
    async def _move_to_archive_table(
        self, policy: RetentionPolicy, cutoff_date: datetime, dry_run: bool
    ) -> Dict[str, Any]:
    ...
    async def restore_archived_data(self, table_name: str, record_ids: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            update_data = {"archived_at": None, "archive_status": ArchiveStatus.ACTIVE.value}

            response = self.supabase.table(table_name).update(update_data).in_("id", record_ids).execute()
    ...
    def update_retention_policy(self, table_name: str, retention_months: int, archive_method: str = "soft_delete"):
        ...
        if table_name in self.retention_policies:
            ...
            logger.info(
                f"Updated retention policy for {table_name}: {retention_months} months, method: {archive_method}"
            )
        else:

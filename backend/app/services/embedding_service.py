...
class EmbeddingService:
    ...
    def __init__(self):
        if settings.OPENAI_API_KEY:
            ...
            self.client = None
        # Use modern embedding model and dimensions from settings
        self.model = settings.EMBEDDING_MODEL
        self.dimension = getattr(settings, "EMBEDDING_DIMENSION", 3072)
        self.rate_limiter = RateLimiter(max_calls=settings.EMBEDDING_RATE_LIMIT or 3000, time_window=60)  # per minute
    ...
    async def generate_embeddings_batch(self, texts: List[str]) -> List[Optional[List[float]]]:
        ...
        try:
            ...
            response = await self.client.embeddings.create(model=self.model, input=valid_texts, encoding_format="float")

            # Map back to original order
            embeddings = {t: emb.embedding for t, emb in zip(valid_texts, response.data)}

            return [embeddings.get(text) if text and text.strip() else None for text in texts]
    ...
    async def embed_code_context(
        self, db: AsyncSession, repository: str, file_path: str, code_content: str, analysis: Dict[str, Any]
    ...
    async def find_similar_documents(
        self, db: AsyncSession, query: str, repository: Optional[str] = None, limit: int = 5, threshold: float = 0.7
        ...
        try:
            result = await db.execute(
                query_text,
                {"embedding": query_embedding, "limit": limit, "threshold": threshold, "repository": repository},
            )
    ...
    async def find_related_code(
        self, db: AsyncSession, query: str, repository: str, limit: int = 5
        ...
        try:
            result = await db.execute(
                ...
                """,
                {"embedding": query_embedding, "repository": repository, "limit": limit},
            )
    ...
    async def detect_duplicates(
        ...
        return [doc for doc, score in similar if score > threshold and (not exclude_id or doc.id != exclude_id)]

    async def update_embeddings_for_repository(self, db: AsyncSession, repository: str, force: bool = False) -> int:
        """Update all embeddings for a repository."""
        # Get all documents without embeddings or force update
        stmt = select(DocEmbedding).where(DocEmbedding.repository == repository)
    ...
    async def create_or_update_embedding(
        self, db: AsyncSession, document_id: str, title: str, content: str, metadata: Optional[Dict[str, Any]] = None
    ) -> Optional[DocEmbedding]:
    ...
    async def batch_create_embeddings(
        self, db: AsyncSession, documents: List[Dict[str, Any]]
    ...
    async def update_code_context_embedding(
        self, db: AsyncSession, repository: str, file_path: str, analysis: Dict[str, Any]
    ) -> Optional[CodeContext]:
        ...
        return await self.embed_code_context(
            db=db, repository=repository, file_path=file_path, code_content=code_content, analysis=analysis
        )

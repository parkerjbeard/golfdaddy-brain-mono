...
from app.core.validators import ReportValidator, DateValidator
...
class SlackConversationHandler:
        ...
        try:
            ...
            if today_report:
                ...
                user_midnight = DateValidator.get_user_midnight_utc(
                    datetime.now(timezone.utc),
                    user_timezone
                ) + timedelta(days=1)  # Next midnight
    ...
    async def handle_modal_submission(self, slack_user_id: str, view_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if callback_id == "eod_report_update":
                ...
                existing_report = await self.daily_report_service.get_daily_report(
                    report_id,
                    user.id
                )
            ...
            try:
                analysis_date = report.report_date.date() if isinstance(report.report_date, datetime) else report.report_date
                ...
                logger.info(f"Unified analysis completed for user {user.id} with {unified_analysis.total_estimated_hours} total hours")
            except Exception as e:
            ...
            if dm_channel:
                ...
                confirm_message = await self._safe_send_message(
                    channel=dm_channel,
                    text=f"✅ I've received your EOD report for today, {user.name}!"
                )
                ...
                clarification_check = await self.daily_report_service.ai_integration.check_if_clarification_needed(
                    report.raw_text_input,
                    report.ai_analysis.model_dump() if report.ai_analysis else {}
                )
        ...
        if action_id == "submit_eod":
            ...
            try:
                ...
                if trigger_id:
                    ...
                    report = await self.daily_report_service.get_report_by_id(
                        UUID(report_id)
                    )
    ...
    def _build_existing_report_blocks(self, report: DailyReport, user: User, can_update: bool = True) -> List[Dict[str, Any]]:
        ...
        if can_update:
            ...
            blocks.append({
                ...
                "elements": [
                    {
                        "type": "mrkdwn",
                        "text": "_You can update your report until midnight in your timezone_"
                    }
                ]
            })
        else:
            blocks.append({
                ...
                "elements": [
                    {
                        "type": "mrkdwn",
                        "text": "_Report updates are closed after midnight_"
                    }
                ]
            })
            ...
            "blocks": [
                {
                    ...
                    "text": {
                        "type": "mrkdwn",
                        "text": f"Hi {user.name}! Update your report for today."
                    }
                },
                {
                    "type": "input",

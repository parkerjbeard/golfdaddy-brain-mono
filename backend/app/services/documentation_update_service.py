...
class DocumentationUpdateService:
    ...
    def __init__(self):
        ...
        if not self.github_token:
            raise ConfigurationError(
                "GitHub token not configured in settings. Documentation features will be disabled."
            )

        if not self.openai_api_key:
            raise ConfigurationError(
                "OpenAI API key not configured in settings. Documentation features will be disabled."
            )
        ...
        try:
            ...
            while contents:
                ...
                if file_content.type == "dir":
                    ...
                    await self.github_rate_limiter.acquire(1)
                    dir_contents = await self.github_circuit_breaker.call(repo.get_contents, file_content.path)
            ...
            if isinstance(e, CircuitBreakerOpenError):
                ...
                raise ExternalServiceError(
                    service_name="GitHub", original_message=f"GitHub service temporarily unavailable: {e}"
                )
                ...
                logger.error(
                    f"GitHub API error fetching repository content for {repo_name}/{path}: {e.status if hasattr(e, 'status') else 'N/A'} {e.data if hasattr(e, 'data') else str(e)}",
                    exc_info=True,
                )
                raise ExternalServiceError(
                    service_name="GitHub",
                    original_message=f"Failed to fetch content from {repo_name}: {e.data.get('message', str(e)) if hasattr(e, 'data') else str(e)}",
                )
        except Exception as e:
    ...
    async def analyze_documentation(
        self, docs_repo_name: str, commit_analysis_result: Dict[str, Any], source_repo_name: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            prompt = self._create_analysis_prompt(
                all_docs_content, doc_files_list, commit_analysis_result, source_repo_name
            )
            ...
            if str(self.openai_model).startswith("gpt-5"):
                response = await self.openai_circuit_breaker.call(
                    ...
                    input=[
                        {
                            "role": "system",
                            "content": "You are an expert technical documentation specialist with a deep understanding of software development. Your task is to analyze existing documentation against recent code changes and suggest necessary updates.",
                        },
                        {"role": "user", "content": prompt},
                    ],
                    response_format={"type": "json_object"},
                )
                # Extract text for Responses API
                content_text = getattr(response, "output_text", None)
                if not content_text and hasattr(response, "choices") and response.choices:
                    content_text = response.choices[0].message.content
                analysis_result = json.loads((content_text or "{}").strip())
            else:
                response = await self.openai_circuit_breaker.call(
            ...
            if isinstance(e, CircuitBreakerOpenError):
                ...
                logger.error(
                    f"OpenAI API error during documentation analysis: {e.status_code if hasattr(e, 'status_code') else 'N/A'} {e.response if hasattr(e, 'response') else str(e)}",
                    exc_info=True,
                )
    ...
    def _create_analysis_prompt(
        self, docs_content: str, doc_files_list: List[str], commit_analysis: Dict[str, Any], source_repo: str
    ) -> str:
        """
...
IMPORTANT: Be conservative in suggesting changes. Only propose documentation updates if they are genuinely needed based on the commit analysis. Most commits will not require documentation updates."""
    ...
    async def create_pull_request(
        ...
        if not proposed_changes:
            ...
            return {"status": "no_changes", "message": "No documentation changes needed"}
        ...
        try:
            ...
            for change in proposed_changes:
                ...
                try:
                    ...
                    await self.github_rate_limiter.acquire(1)
                    file_content = await self.github_circuit_breaker.call(repo.get_contents, file_path, ref=branch_name)
                    ...
                    if str(self.openai_model).startswith("gpt-5"):
                        updated_content_response = await self.openai_circuit_breaker.call(
                            ...
                            input=[
                                {
                                    "role": "system",
                                    "content": "You are a technical documentation specialist. Your task is to update markdown documentation to accurately reflect code changes.",
                                },
                                {"role": "user", "content": update_prompt},
                            ],
                        )
                        updated_content = (getattr(updated_content_response, "output_text", None) or "").strip()
                        if (
                            not updated_content
                            and hasattr(updated_content_response, "choices")
                            and updated_content_response.choices
                        ):
                    ...
                    logger.error(
                        f"GitHub error updating file {file_path} in {branch_name}: {e_git_file.status} {e_git_file.data}",
                        exc_info=True,
                    )
                    change["error"] = (
                        f"Failed to update file on GitHub: {e_git_file.data.get('message', str(e_git_file))}"
                    )
                    continue
                except OpenAIAPIError as e_openai_file:
                    logger.error(
                        f"OpenAI error generating content for file {file_path}: {e_openai_file.status_code} {e_openai_file.response}",
                        exc_info=True,
                    )
                    change["error"] = (
                        f"Failed to generate updated content with OpenAI: {e_openai_file.message or str(e_openai_file)}"
                    )
                    continue
            ...
            pr = await self.github_circuit_breaker.call(
                repo.create_pull, title=pr_title, body=pr_body, head=branch_name, base=default_branch
            )
            ...
            if isinstance(e, CircuitBreakerOpenError):
                ...
                raise ExternalServiceError(
                    service_name=service_name, original_message=f"{service_name} service temporarily unavailable: {e}"
                )
            elif isinstance(e, RateLimitExceededError):
                ...
                raise ExternalServiceError(
                    service_name=service_name, original_message=f"{service_name} rate limit exceeded: {e}"
                )
            elif isinstance(e, GithubException):
                logger.error(
                    f"GitHub API error creating pull request for {docs_repo_name}: {e.status} {e.data}", exc_info=True
                )
                raise ExternalServiceError(
                    service_name="GitHub",
                    original_message=f"Failed to create PR for {docs_repo_name}: {e.data.get('message', str(e))}",
                )
            else:  # OpenAIAPIError
                logger.error(
                    f"OpenAI API error during PR creation process for {docs_repo_name}: {e.status_code} {e.response}",
                    exc_info=True,
                )
    ...
    async def save_to_git_repository(
        self, repo_name: str, file_path: str, content: str, title: str, commit_message: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            try:
                await self.github_rate_limiter.acquire(1)
                file_content = await self.github_circuit_breaker.call(repo.get_contents, file_path, ref=default_branch)
            ...
            if file_exists:
                ...
                await self.github_rate_limiter.acquire(1)
                result = await self.github_circuit_breaker.call(
                    repo.create_file, path=file_path, message=commit_message, content=content, branch=default_branch
                )
                file_url = f"https://github.com/{repo_name}/blob/{default_branch}/{file_path}"
            ...
            if isinstance(e, CircuitBreakerOpenError):
                ...
                logger.error(
                    f"GitHub API error saving to repository {repo_name}/{file_path}: {e.status} {e.data}", exc_info=True
                )
                raise ExternalServiceError(
                    service_name="GitHub",
                    original_message=f"Failed to save to Git repo {repo_name}: {e.data.get('message', str(e))}",
                )
        except Exception as e:
    ...
    async def analyze_commit_and_suggest_updates(
        self, commit: Any, docs_content: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """
    ...
    async def scan_repository_for_updates(self, repo_name: str, days_back: int = 7) -> Dict[str, Any]:
        """
    ...
    async def create_documentation_pr(
        self, repo_name: str, updates: List[Dict[str, Any]], branch_name: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for update in updates:
                ...
                try:
                    ...
                    repo.update_file(file_path, f"Update {file_path}", content, file_content.sha, branch=branch_name)
                except GithubException:
                    # File doesn't exist, create it
                    repo.create_file(file_path, f"Create {file_path}", content, branch=branch_name)

...
class DocCacheService:
    ...
    def __init__(self):
        ...
        logger.info(
            f"DocCacheService initialized with TTL={self.default_ttl_seconds}s, "
    ...
    async def _evict_lru_entries(self):
        ...
        sorted_entries = sorted(self._memory_cache.items(), key=lambda x: x[1].last_accessed or x[1].created_at)
...
def cached(operation: str, ttl_seconds: Optional[int] = None, key_params: Optional[List[str]] = None):
    """
...
class CacheWarmupService:
        ...
        try:
            ...
            for doc_type in common_doc_types:
                ...
                await self.cache_service.set(
                    "quality_template", template_data, ttl_seconds=7200, doc_type=doc_type  # 2 hours
                )
    ...
    def _get_quality_threshold(self, doc_type: str) -> float:
        ...
        thresholds = {"api": 85.0, "guide": 80.0, "reference": 90.0, "tutorial": 85.0}

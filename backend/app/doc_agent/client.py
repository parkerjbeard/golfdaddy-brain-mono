...
class AutoDocClient:
    ...
    ) -> None:  # Removed openai_model parameter
        ...
        async def _call_openai() -> Any:
            if str(self.openai_model).startswith("gpt-5"):
                return await self.openai_client.responses.create(
                    ...
                    input=[{"role": "system", "content": prompt}, {"role": "user", "content": diff}],
                )
        ...
        if not db:
            async with get_db() as session:
                ...
                return await self._create_and_send_approval(
                    db, diff, patch, commit_hash, commit_message, files_affected, additions, deletions
                )
        else:
            return await self._create_and_send_approval(
                db, diff, patch, commit_hash, commit_message, files_affected, additions, deletions
    ...
    async def analyze_diff_with_context(
        self, diff: str, repo_path: str, commit_hash: str, db: Optional[AsyncSession] = None
    ) -> str:
    ...
    async def _gather_context(
        self, db: Optional[AsyncSession], repository: str, diff: str, commit_hash: str
    ) -> Dict[str, Any]:
        ...
        for file_path in context["affected_files"][:5]:  # Limit to first 5 files
            file_context = await self.context_analyzer.get_file_context(db, repository, file_path)

            if file_context:
                # Add patterns and dependencies
                context["code_patterns"].extend(file_context.get("design_patterns", []))
                context["dependencies"].extend(file_context.get("dependencies", []))
        ...
        if self.embedding_service:
            ...
            related = await self.embedding_service.find_similar_documents(
                db, change_summary, repository, limit=3, threshold=0.7
            )

            context["related_docs"] = [
                {"title": doc.title, "content": doc.content[:500], "similarity": score, "file_path": doc.file_path}
                for doc, score in related
            ]

            # Check for duplicate documentation
            duplicates = await self.embedding_service.detect_duplicates(
                db, f"Changes from {commit_hash}", change_summary, repository
            )

            if duplicates:
                context["potential_duplicates"] = [
                    {"title": dup.title, "file_path": dup.file_path} for dup in duplicates
                ]
    ...
    def _build_context_aware_prompt(self, diff: str, context: Dict[str, Any]) -> str:
        ...
        if context.get("code_patterns"):
            prompt_parts.append(f"- Design patterns used: {', '.join(context['code_patterns'][:5])}")

        if context.get("dependencies"):
            prompt_parts.append(f"- Key dependencies: {', '.join(context['dependencies'][:5])}")

        if context.get("related_docs"):
            prompt_parts.extend(["", "RELATED DOCUMENTATION (for reference):"])
            for doc in context["related_docs"][:2]:
                prompt_parts.extend(
                    [f"- {doc['title']} (similarity: {doc['similarity']:.2f})", f"  Preview: {doc['content'][:200]}..."]
                )
    ...
    async def check_documentation_coverage(self, db: AsyncSession, repository: str, file_path: str) -> Dict[str, Any]:
        ...
        if related_docs:
            coverage_info["documentation_files"] = [
                {"title": doc.title, "path": doc.file_path, "relevance": score} for doc, score in related_docs
            ]
        ...
        if file_context.get("classes") and coverage_info["coverage_score"] < 0.8:
            coverage_info["suggestions"].append(f"Document classes: {', '.join(file_context['classes'][:3])}")

        if file_context.get("functions") and len(file_context["functions"]) > 5:
            coverage_info["suggestions"].append(f"Add API documentation for {len(file_context['functions'])} functions")

        if not coverage_info["has_documentation"]:
            coverage_info["suggestions"].append(f"Create initial documentation for {file_path}")

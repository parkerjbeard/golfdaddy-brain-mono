...
class AutoDocClientV2:
    ...
    def __init__(
        ...
        if use_github_app and all(
            [settings.GITHUB_APP_ID, settings.GITHUB_APP_PRIVATE_KEY, settings.GITHUB_APP_INSTALLATION_ID]
        ):
    ...
    def get_commit_diff_stats(self, diff: str) -> Dict[str, int]:
        ...
        try:
            ...
            result = subprocess.run(
                ["git", "diff", "--numstat", "--no-index", "/dev/null", temp_path], capture_output=True, text=True
            )
            ...
            return {"files_affected": files_affected, "additions": additions, "deletions": deletions}
    ...
    async def create_pr_with_check_run(
        self, diff: str, commit_hash: str, branch_name: Optional[str] = None, approval_id: Optional[str] = None
        ...
        try:
            ...
            base_ref = self.github_app.get_file_contents(owner, repo, "README.md", ref=default_branch)
            ...
            for file_path, content in files_to_update.items():
                ...
                try:
                    existing = self.github_app.get_file_contents(owner, repo, file_path, ref=default_branch)
            ...
            # Create pull request
            pr_title = f"Automated docs update for {commit_hash[:7]}"
            pr_body = (
                f"This PR contains automated documentation updates for commit {commit_hash}.\n\n"
                f"Approval ID: {approval_id}\n\n"
                f"Generated by AutoDoc"
            )
            pr_data = self.github_app.create_pull_request(
                owner, repo, title=pr_title, head=branch_name, base=default_branch, body=pr_body, draft=False
            )

            # Add labels and request reviewers if configured
            try:
                labels = ["documentation", "auto-docs"]
                self.github_app.add_labels_to_pr(owner, repo, pr_data["number"], labels)
            except Exception:
                pass

            try:
                reviewers_env = os.getenv("DOCS_PR_REVIEWERS")  # comma-separated GitHub usernames
                reviewers = [r.strip() for r in reviewers_env.split(",") if r.strip()] if reviewers_env else []
                if reviewers:
                    self.github_app.request_reviewers(owner, repo, pr_data["number"], reviewers=reviewers)
            except Exception:
                pass

            # Post a summary comment
            try:
                summary = "Automated documentation updates proposed. Please review the changes."
                self.github_app.create_issue_comment(owner, repo, pr_data["number"], summary)
            except Exception:
                pass
            ...
            # Include requested_actions for GitHub UI
            check_run = self.github_app.create_check_run(
                ...
                actions=[
                    {"label": "Approve Docs", "description": "Mark docs approved", "identifier": "approve_docs"},
                    {
                        "label": "Regenerate Patch",
                        "description": "Ask AI to regenerate",
                        "identifier": "regenerate_patch",
                    },
                ],
            )
    ...
    async def update_check_run_status(
        ...
        try:
            ...
            self.github_app.update_check_run(
                owner, repo, check_run_id, status=status, conclusion=conclusion, output=output
            )
    ...
    async def propose_via_slack(
        self, diff: str, patch: str, commit_hash: str, commit_message: str = "", db: Optional[AsyncSession] = None
    ) -> Optional[str]:
        """
    ...
    async def _create_and_send_approval(
        ...
        try:
            ...
            pr_result = await self.create_pr_with_check_run(patch, commit_hash, approval_id=str(approval.id))
            ...
            result = await self.slack_service.send_message(
                channel=self.slack_channel, text=message["text"], blocks=message["blocks"]
            )

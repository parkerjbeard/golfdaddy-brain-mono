...
async def handle_doc_approval(
    ...
    try:
        ...
        result = await db.execute(
            select(DocApproval).where(DocApproval.id == approval_id)
        )
        ...
        if approval.status != "pending":
            return {
                "response_type": "ephemeral",
                "text": f"⚠️ This request has already been {approval.status}."
            }
        ...
        # Create PR using doc agent
        # TODO: Fix doc_agent import path
        ...
        # pr_url = client.apply_patch(approval.patch_content, approval.commit_hash)
        pr_url = None  # Temporary placeholder
        ...
        if pr_url:
            ...
            if channel and message_ts:
                ...
                if original_blocks:
                    original_blocks[0] = {
                        ...
                        "text": {
                            "type": "plain_text",
                            "text": "✅ Documentation Update Approved",
                            "emoji": True
                        }
                    }
                    # Add approval info
            ...
            return {
                "response_type": "ephemeral",
                "text": "❌ Failed to create PR. Please check the logs."
            }
...
async def handle_doc_rejection(
    approval_id: str, 
    payload: Dict[str, Any], 
    db: AsyncSession
) -> Dict[str, Any]:
    ...
    try:
        ...
        # For now, still update the approval record
        ...
        if channel and message_ts:
            ...
            if original_blocks:
                original_blocks[0] = {
                    ...
                    "text": {
                        "type": "plain_text",
                        "text": "❌ Documentation Update Rejected",
                        "emoji": True
                    }
                }
                # Add rejection info
        ...
        return {
            "response_type": "in_channel",
            "text": f"❌ Documentation update rejected by {user_name}."
        }
...
async def handle_view_full_diff(
    approval_id: str, 
    payload: Dict[str, Any], 
    db: AsyncSession
) -> Dict[str, Any]:
    ...
    try:
            ...
            modal_view = {
                ...
                "blocks": [
                    ...
                    },
                    {
                        ...
                        "text": {
                            "type": "mrkdwn",
                            "text": f"```{approval.diff_content[:2900]}```"  # Slack limits
                        }
                    }
                ]
            }
        ...
        return {
            "response_type": "ephemeral",
            "text": "❌ An error occurred while retrieving the diff."
        }

...
async def get_user_kpi_summary(
    user_id: UUID,
    periodDays: Optional[int] = Query(7, ge=1, le=90, description="Number of days for the summary period, ending today."),
    startDate: Optional[date] = Query(None, description="Start date for the summary period (YYYY-MM-DD). Overrides periodDays if provided."),
    endDate: Optional[date] = Query(None, description="End date for the summary period (YYYY-MM-DD). Defaults to today if startDate is provided without endDate."),
    kpi_service: KpiService = Depends(get_kpi_service),

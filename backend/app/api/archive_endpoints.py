...
async def run_archiving(
    ...
    try:
        logger.info(
            f"Admin {current_user.email} initiated archiving: table={request.table_name}, dry_run={request.dry_run}"
        )

        results = await archive_service.archive_old_data(table_name=request.table_name, dry_run=request.dry_run)
...
async def restore_archived_data(
    ...
    try:
        logger.info(
            f"Admin {current_user.email} initiated restoration: table={request.table_name}, count={len(request.record_ids)}"
        )

        result = await archive_service.restore_archived_data(
            table_name=request.table_name, record_ids=request.record_ids
        )
...
async def get_archive_statistics(
    archive_service: ArchiveService = Depends(get_archive_service), current_user: User = Depends(require_admin_user)
):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to get archive statistics"
        )
...
async def get_retention_policies(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to get retention policies"
        )
...
async def update_retention_policy(
    ...
    try:
        logger.info(
            f"Admin {current_user.email} updating retention policy: {request.table_name} -> {request.retention_months} months"
        )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to update retention policy"
        )
...
async def purge_archived_data(
    ...
    try:
        logger.warning(
            f"Admin {current_user.email} initiated PURGE operation: table={table_name}, older_than={older_than_months} months"
        )

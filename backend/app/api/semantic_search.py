...
async def search_documents(
    request: SearchRequest, db: AsyncSession = Depends(get_db), current_user: dict = Depends(get_current_user)
):
    """
...
async def get_related_documents(
    ...
    try:
        related = await search_service.find_related_documentation(db, document_id, limit)

        return {"document_id": document_id, "related_documents": related, "count": len(related)}
...
async def analyze_documentation_gaps(
    ...
    try:
        gaps = await search_service.analyze_documentation_gaps(db, repository, threshold)
...
async def get_documentation_graph(
    ...
    try:
        graph = await search_service.get_documentation_graph(db, repository, max_nodes)
...
async def get_improvement_suggestions(
    document_id: str, db: AsyncSession = Depends(get_db), current_user: dict = Depends(get_current_user)
):
    """Get AI-powered improvement suggestions for a document."""
    search_service = SemanticSearchService()

    try:
        suggestions = await search_service.suggest_documentation_improvements(db, document_id)
...
async def analyze_repository_context(
    ...
    try:
        analysis = await context_analyzer.analyze_repository(db, repository, repo_path, max_files)
...
async def get_documentation_coverage(
    repository: str, db: AsyncSession = Depends(get_db), current_user: dict = Depends(get_current_user)
):

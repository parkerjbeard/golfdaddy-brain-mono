...
def verify_webhook_signature(
    ...
    if signature and settings.ZAPIER_WEBHOOK_SECRET:
        expected_signature = hmac.new(settings.ZAPIER_WEBHOOK_SECRET.encode(), request_body, hashlib.sha256).hexdigest()
...
async def social_media_views_webhook(request: Request, db: Session = Depends(get_db)):
...
async def csat_feedback_webhook(request: Request, db: Session = Depends(get_db)):
    ...
    return {"status": "success", "id": str(feedback.id), "message": "User feedback recorded"}


@router.post("/current-objective")
async def current_objective_webhook(request: Request, db: Session = Depends(get_db)):
    ...
    return {"status": "success", "id": str(objective.id), "message": f"Objective '{objective.title}' saved"}


@router.post("/wins-section")
async def wins_section_webhook(request: Request, db: Session = Depends(get_db)):
    ...
    return {"status": "success", "id": str(win.id), "message": f"Win '{win.title}' recorded"}


@router.post("/analytics")
async def analytics_webhook(request: Request, db: Session = Depends(get_db)):
...
async def google_forms_webhook(request: Request, db: Session = Depends(get_db)):
...
async def employee_directory_webhook(request: Request, db: Session = Depends(get_db)):
...
async def test_webhook(request: Request, db: Session = Depends(get_db)):
    ...
    try:
        ...
        return {"status": "error", "message": str(e.detail), "timestamp": datetime.utcnow().isoformat()}

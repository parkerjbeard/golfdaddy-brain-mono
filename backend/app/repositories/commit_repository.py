...
class CommitRepository:
            ...
            if response.data:
                ...
                if 'ai_estimated_hours' in saved_data and saved_data['ai_estimated_hours'] is not None:
                    try:
                        ...
                        logger.warning(f"Could not convert/round ai_estimated_hours in save_commit: {saved_data.get('ai_estimated_hours')}. Error: {conversion_exc}", exc_info=True)
                        pass 
                ...
                for saved_data_item in response.data:
                     if 'ai_estimated_hours' in saved_data_item and saved_data_item['ai_estimated_hours'] is not None:
                         try:
                             ...
                             logger.warning(f"Could not convert/round ai_estimated_hours in bulk_insert_commits for an item. Value: {saved_data_item.get('ai_estimated_hours')}. Error: {conversion_exc}", exc_info=True)
                             pass 
            ...
            if response.data:
                ...
                if 'ai_estimated_hours' in saved_data and saved_data['ai_estimated_hours'] is not None:
                    try:
                        ...
                        logger.warning(f"Could not convert/round ai_estimated_hours in get_commit_by_hash: {saved_data.get('ai_estimated_hours')}. Error: {conversion_exc}", exc_info=True)
                        pass 
                return Commit(**saved_data)
    ...
    async def update_commit_analysis(self, commit_hash: str, ai_estimated_hours: float, seniority_score: int = None) -> Optional[Commit]:
            ...
            if response.data:
                ...
                if 'ai_estimated_hours' in saved_data and saved_data['ai_estimated_hours'] is not None:
                    try:
                        ...
                        logger.warning(f"Could not convert/round ai_estimated_hours in update_commit_analysis: {saved_data.get('ai_estimated_hours')}. Error: {conversion_exc}", exc_info=True)
                        pass 
                return Commit(**saved_data)
            else:
                ...
                logger.error(f"Failed to update commit analysis for {commit_hash}: No data returned and no Supabase error. Commit might exist but update failed.")
                raise DatabaseError(f"Failed to update commit analysis for {commit_hash}: Update operation returned no data.")
    ...
    async def get_commits_by_user_in_range(self, author_id: UUID, start_date: date, end_date: date) -> List[Commit]:
        ...
        try:
            ...
            response: PostgrestAPIResponse = await asyncio.to_thread(
                self._client.table(self._table).select("*")\
                ...
                for saved_data_item in response.data:
                     if 'ai_estimated_hours' in saved_data_item and saved_data_item['ai_estimated_hours'] is not None:
                         try:
                             ...
                             logger.warning(f"Could not convert/round ai_estimated_hours in get_commits_by_user_in_range for an item. Value: {saved_data_item.get('ai_estimated_hours')}. Error: {conversion_exc}", exc_info=True)
                             pass 
                     saved_commits.append(Commit(**saved_data_item))
                return saved_commits
            else:
    ...
    async def delete_commit(self, commit_hash: str) -> bool:
        ...
        try:
            ...
            if hasattr(e, 'details'):
                logger.error(f"Supabase error details during delete: {getattr(e, 'details')}", exc_info=False) # exc_info=False to avoid duplicate main traceback
            ...
            if response.data:
                ...
                if 'ai_estimated_hours' in saved_data and saved_data['ai_estimated_hours'] is not None:
                    try:
                        ...
                        logger.warning(f"⚠ Could not convert/round ai_estimated_hours in create_commit: {saved_data.get('ai_estimated_hours')}. Error: {conversion_exc}", exc_info=True)
                        pass
                ...
                raise DatabaseError(f"Failed to create commit {commit_hash}: No data returned and no Supabase error object.")
        except DatabaseError:
            raise
        except Exception as e:

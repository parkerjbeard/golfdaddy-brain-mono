...
class DailyWorkAnalysisRepository:
    ...
    async def get_by_id(self, analysis_id: UUID) -> Optional[DailyWorkAnalysis]:
        ...
        try:
            ...
            response: PostgrestAPIResponse = self._client.table(self._table)\
                .select("*")\
            ...
            # Get work items
            work_items_response = self._client.table(self._work_items_table)\
                .select("*")\
            ...
            dedup_response = self._client.table(self._dedup_table)\
                .select("*")\
                .eq("daily_analysis_id", str(analysis_id))\
                .execute()
    ...
    async def get_analyses_for_date_range(
        ...
        try:
            logger.info(
                f"Fetching daily work analyses for user {user_id} "
    ...
    async def get_weekly_aggregate(
        ...
        try:
            ...
            logger.info(
                f"Fetching weekly aggregate for user {user_id} "
    ...
    async def get_monthly_aggregate(
        ...
        try:
            ...
            logger.info(
                f"Fetching monthly aggregate for user {user_id} "
    ...
    async def _update_work_items(self, analysis_id: str, work_items: List[Dict[str, Any]]):
        ...
        try:
            ...
            delete_response = self._client.table(self._work_items_table)\
                .delete()\
                .eq("daily_analysis_id", analysis_id)\
                .execute()
    ...
    async def _update_deduplication_results(self, analysis_id: str, dedup_results: List[Dict[str, Any]]):
        ...
        try:
            ...
            delete_response = self._client.table(self._dedup_table)\
                .delete()\
                .eq("daily_analysis_id", analysis_id)\
                .execute()
    ...
    async def update_processing_status(
        ...
        try:
            ...
            self._handle_supabase_error(
                response, 
                f"Failed to update processing status for {analysis_id}"
            )
